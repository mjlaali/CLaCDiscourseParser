...
public class DiscourceVsNonDiscourseEntropy {
	...
	private Instances prunedInstances;
	...
	public DiscourceVsNonDiscourseEntropy(File arffFile, int minFreq) throws Exception {
		...
		getPruned(minFreq);
		WekaToDistribution wekaToDistribution = new WekaToDistribution(prunedInstances);
	...
	private void getPruned(int minCount) throws Exception {
		...
		prunedInstances = Filter.useFilter(instances, filter);
	}
	...
	public static void main(String[] args) throws Exception {
		...
		connectiveEntropy.stream().forEach((p) -> System.out.printf("%s\t%f\t%d\n", p.getFirst(), p.getSecond(), freq.get(p.getFirst())));
	}
